...
Set up R/Bioconductor
-------------------------------------------------------------

This codelab `can be run from anywhere <https://github.com/googlegenomics/bioconductor-workshop-r>`_.  All the heavy lifting is happening in the cloud regardless of where R is running.  Here are three options to set up your environment

Run the Bioconductor Docker container on Google Compute Engine
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
But to further the goals of `reproducibility, ease of use, and convenience <http://www.bioconductor.org/help/docker/>`_, you can run this codelab in a Bioconductor Docker container deployed to `Google Compute Engine`_.


You now have access to RStudio via your browser!

Run the Bioconductor Docker container locally
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
.. container:: toggle
...
Configure your local R installation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* `Data Analysis using Google Genomics <https://github.com/googlegenomics/bioconductor-workshop-r/blob/master/inst/doc/AllModalitiesDemo.md>`_, also available on YouTube:
